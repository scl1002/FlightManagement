...
####### Connection
  ...
  def load_sample_data(self):
    try:
      self.get_connection()
      self.cur.executemany("Insert of ignore into Destinations (DestinationID, City, Country, AirportCode, AirportName, Timezone) values (?, ?, ?, ?, ?, ?)", self.sample_destinations)
      self.cur.executemany("Insert of ignore into Pilots (PilotID, FirstName, LastName, LicenseNumber, Rank, FlightHours) values (?, ?, ?, ?, ?, ?)", self.sample_pilots)
      self.cur.executemany("Insert of ignore into Flights (FlightID, Origin, DestinationID, DepartureDate, DepartureTime, ArrivalTime, Status, PilotID) values (?, ?, ?, ?, ?, ?, ?, ?)", self.sample_flights)
      self.conn.commit()
